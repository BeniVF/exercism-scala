...
object Alphametics {
  ...
  private def solveExpression(uniqueLetters: String)(
      boolExpr: Expression[Boolean]): Option[Solution] = {
...
case class Equals[A](override val left: Expression[A], override val right: Expression[A])
    extends Operation[A, Boolean] {
  override val op: (A, A) => Boolean = _ == _
}

case class Plus(override val left: Expression[Long], override val right: Expression[Long])
    extends Operation[Long, Long] {
  override val op: (Long, Long) => Long = _ + _
}

case class Mult(override val left: Expression[Long], override val right: Expression[Long])
    extends Operation[Long, Long] {
  override val op: (Long, Long) => Long = _ * _
}

case class Power(override val left: Expression[Long], override val right: Expression[Long])
    extends Operation[Long, Long] {
