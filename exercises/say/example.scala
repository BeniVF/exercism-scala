object Say {
  ...
  private val tens =
    IndexedSeq("", "", "twenty", "thirty", "forty", "fifty", "sixty", "seventy", "eighty", "ninety")
  ...
  def inEnglish(n: Long): Option[String] = {
    ...
    val translated = zipped.map {
      case (i: Int, s: String) => if (i > 0) s + " " + scales(i) else s
    }
  ...
  private def translateChunk(n: Int): String = {
    ...
    val w1 =
      if (hundredsDigit > 0) smallNums(hundredsDigit) + " " + scales.head
      else ""

    val w2 =
      if (remainder > 19)
        tens(remainder / 10) + smallNums.get(remainder % 10).map("-" + _).getOrElse("")
