    ...
    def fromLabeledTest(argNames: String*): ToTestCaseData =
      withLabeledTest { sut => labeledTest =>
        ...
        val sutCall = rollVal match {
          case Some(r) =>
            s"""val score = ${previousRollsVal}.foldLeft(Bowling())((acc, roll) => acc.roll(roll)).roll($r).score()
    score$isDefined"""
          case None =>
            s"""val score = ${previousRollsVal}.foldLeft(Bowling())((acc, roll) => acc.roll(roll)).score()
    score$isDefined"""
        }
