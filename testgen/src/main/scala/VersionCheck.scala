...
object VersionCheck {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length != 1) {
      println(
        "Usage VersionCheck /path/ where path is the filesystem path to the problem-specifications exercises directory.")
      System.exit(-1)
    }
        ...
        case x :: _ =>
          ...
          if (canonicalDataPath.exists()) {
            ...
            if (!implementedVersion.equals(canonicalVersion)) {
              println(
                s"${f.getName}\timplemented version - $implementedVersion\tcanonical version - $canonicalVersion")
            }
          }
        case _ =>
      }
