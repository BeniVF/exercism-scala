...
object KindergartenGardenTestGenerator {
  def main(args: Array[String]): Unit = {
    ...
    def toString(expected: CanonicalDataParser.Expected): String =
      expected match {
        case Right(xs: List[String]) =>
          s"List(${xs.map(x => "Plant." + x.capitalize).mkString(", ")})"
      ...
      withLabeledTest { sut => labeledTest =>
        ...
        val sutCall =
          if (students.isEmpty)
            ...
            s"""Garden(List(${students.map(s => escape(s)).mkString(", ")}), ${escape(
              diagram.toString)}).$property(${escape(student.toString)})"""
