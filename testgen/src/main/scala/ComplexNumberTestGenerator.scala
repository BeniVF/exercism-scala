  ...
  def main(args: Array[String]): Unit = {
    def toString(expected: CanonicalDataParser.Expected): String =
      expected match {
        ...
        case Right(xs: List[Any]) =>
          s"""ComplexNumber(${toStringArg(xs.head)}, ${toStringArg(xs.tail.head)})"""
    ...
    def createRealTestCase(labeledTest: LabeledTest) = {
      ...
      val sutCall =
        s"""ComplexNumber(real = ${zArg._1}).${labeledTest.property} should be (${toString(
          labeledTest.expected)})"""
      TestCaseData(labeledTest.description, sutCall, "")
    }
    ...
    def createImaginaryTestCase(labeledTest: LabeledTest) = {
      val zArg = getZArg(labeledTest)
      val sutCall =
        s"""ComplexNumber(imaginary = ${zArg._2}).${labeledTest.property} should be (${toString(
          labeledTest.expected)})"""
      TestCaseData(labeledTest.description, sutCall, "")
    }
    ...
    def createAbsTestCase(labeledTest: LabeledTest) = {
      val zArg = getZArg(labeledTest)
      val sutCall =
        s"""ComplexNumber(${zArg._1}, ${zArg._2}).${labeledTest.property} should be (${toString(
          labeledTest.expected)})"""
      TestCaseData(labeledTest.description, sutCall, "")
    }
    ...
    def createExpTestCase(labeledTest: LabeledTest) = {
      val zArg = getZArg(labeledTest)
      val sutCall =
        s"""val result = ComplexNumber.${labeledTest.property}(ComplexNumber(${toStringArg(zArg._1)}, ${toStringArg(
             zArg._2)}))
           |assertEquals(result, ${toString(labeledTest.expected)})""".stripMargin
      TestCaseData(labeledTest.description, sutCall, "")
    }
